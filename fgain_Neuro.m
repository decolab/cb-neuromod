...
for we = WE
 for ii = 1:20
    ...
    [hh pp FCDfitt5(ii)] = kstest2(cotsampling5,cotsamplingsim);    
    [hh pp FCDfitt2(ii)] = kstest2(cotsampling2,cotsamplingsim);    
    
    FCsimul = squeeze(mean(FCs,1));
    
    cc = corrcoef(FCemp5(Isubdiag),FCsimul(Isubdiag));
    fitting5(ii) = cc(2);
  
    cc = corrcoef(FCemp2(Isubdiag),FCsimul(Isubdiag));
    fitting2(ii) = cc(2);
  end    
end
