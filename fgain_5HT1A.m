...
%%  OPTIMIZATION LSD RECEPTOR 5HT-1A
%
%
...
Receptor = symm_mean5HT1A(:,1)/max(symm_mean5HT1A(:,1));
...
for nsub = 1:NSUB
    ...
    for seed = 1:N
        ...
        x(find(x>3*std(x)))  = 3*std(x);                    % Remove strong artefacts
...
FCDfitt_rcp1A = FCDfitt;
fitting_rcp1A = fitting;

save('fgain_rcp1A.mat','WG','FCDfitt_rcp1A','fitting_rcp1A');
