...
%%  GENERALIZATION CAPABILITY: OPTIMIZATION LSD TRAINING SET (50% OF THE SUBJECTS)
%
...
%% Define Trainning and Test sets

numsub   = 1:15;
SUBJ     = randperm(15,7);      % 7 is half of the subjects
testSUBJ = randsample(setdiff(numsub,SUBJ),7);
NSUB     = size(SUBJ,2);
...
kk3 = 1;
for nsub = 1:NSUB
    signaldata = eval(sprintf('TC%d', SUBJ(nsub)));
...
FCDfitt_train = FCDfitt;
fitting_train = fitting;

save('LSD_training.mat','WG','FCDfitt_train','fitting_train');
save('testSUBJ.mat','SUBJ','testSUBJ')
